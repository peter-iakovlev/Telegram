    ...
    TGDispatchOnMainThread(^
    {
        ...
        if (messageIds.count != 0)
        {
            ...
            if (encryptedConversationId != 0 && randomIds.count != 0)
            {
                ...
                if (messageData != nil)
                {
                    [TGModernSendSecretMessageActor enqueueOutgoingServiceMessageForPeerId:self.peerId layer:MIN(peerLayer, [TGModernSendSecretMessageActor currentLayer]) randomId:actionRandomId messageData:messageData];
                }
