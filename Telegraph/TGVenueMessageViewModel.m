...
{
    _venueNameModel.frame = CGRectMake(_imageModel.frame.size.width + 6.0f, headerHeight + 4.0f, _venueNameModel.frame.size.width, _venueNameModel.frame.size.height);
    ...
    _imageModel.frame = CGRectMake(_backgroundModel.frame.origin.x + 6.0f + (_incomingAppearance ? 5.0f : 0.0f), _backgroundModel.frame.origin.y + 6.0f + headerHeight, _imageModel.frame.size.width, _imageModel.frame.size.height);
}
...
- (CGSize)contentSizeForContainerSize:(CGSize)containerSize needsContentsUpdate:(bool *)needsContentsUpdate hasDate:(bool)__unused hasDate hasViews:(bool)__unused hasViews
{
    ...
    bool updateNameContents = [_venueNameModel layoutNeedsUpdatingForContainerSize:nameContainerSize layoutFlags:layoutFlags];
    ...
    bool updateAddressContents = [_venueAddressModel layoutNeedsUpdatingForContainerSize:addressContainerSize layoutFlags:layoutFlags];
    ...
    *needsContentsUpdate = updateNameContents || updateAddressContents;
    ...
    return CGSizeMake(MAX(nameWidth, venueWidth) + imageSize.width + 14.0f, imageSize.height + 2.0f);
}
